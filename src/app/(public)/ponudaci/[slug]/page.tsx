...
import { getListingBySlug, getRelatedListings, getCategoriesWithCounts } from "@/lib/queries";
...
import { CompareToggle } from "@/components/favorite-button";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!listing) return {};
  return {
    title: listing.seoTitle ?? `${listing.name}${listing.baseLocation ? ` — ${listing.baseLocation.name}` : ""}`,
    description:
      listing.seoDescription ??
      `${listing.shortDescription} Pogledaj ponudu, cijene i pošalji izravan upit preko Feštka.`,
    alternates: {
      canonical: listing.canonicalOverride ?? absoluteUrl(`/ponudaci/${slug}`),
    },
  };
}
...
export default async function ListingPage({ params }: Props) {
  ...
  await recordListingView(listing.id, `/ponudaci/${slug}`);
    ...
    <>
      ...
      <JsonLd
        data={{
          ...
          url: absoluteUrl(`/ponudaci/${listing.slug}`),
